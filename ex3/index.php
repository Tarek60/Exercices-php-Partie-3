<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <title>Exercice 3</title>
</head>
<body>
  <?php
  $firstVariable = 100; //initialisation de la premiere variable
  $secondVariable = 42; //initialisation de la deuxieme variable
  while ($firstVariable >= 10) { //Tant que (la première variable n'est pas inférieur ou égale à 10)
    $result = $firstVariable * $secondVariable; //multiplie la premiere variable avec la deuxieme
    echo $result; //on affiche le resultat
    ?>
    <br> <!--pour revenir a la ligne-->
    <?php
    $firstVariable--; //on decremente la premiere variable de 1
  }
  ?>
</body>
</html>
